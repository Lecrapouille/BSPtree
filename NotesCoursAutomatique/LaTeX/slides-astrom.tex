...
\newpage
\section{Etude complËte d'un pendule inversÈ sur un chariot}
\dessin{penduleinversechariot}{0.8}{ModËle}{pendulechariot}

\subsection{Le modËle}
\subsubsection{Les Ènergies}
L'Ènergie cinÈtique du chariot : $$\EE_c^{chariot} = 1/2 M \dot{x}^2.$$

L'Ènergie potentiel du chariot : $$\EE_p^{chariot} = u x.$$

L'Ènergie cinÈtique du penduke : $$\EE_c^{pendule} = 1/2 m ((\dot{x+l sin\theta})^2 + (\dot{l cos \theta})^2),$$

$$= 1/2 m (\dot{x}^2 + l^2 \dot{\theta} + 2\dot{x}\dot{\theta} l cos \theta.$$

L'Ènergie potentiel du pendule : $$\EE_p^{pendule} = mgl cos \theta$$
